...
const UploadSection = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!flaskResponse.ok) {
        throw new Error("Failed to process image on the Flask server");
      }

      const imageBlob = await flaskResponse.blob();
      const processedImageUrl = URL.createObjectURL(imageBlob);
      setProcessedImage(processedImageUrl); // Display locally
      ...
      if (!fetchProcessedUrlResponse.ok) {
        throw new Error("Failed to fetch upload URL for processed image");
      }
      ...
      if (!processedUploadURL) {
        throw new Error("No upload URL received for processed image");
      }
      ...
      const processedUploadResponse = await fetch(processedUploadURL, {
        method: "PUT",
        headers: {
          "Content-Type": "image/png", // Adjust based on Flask output
        },
        body: imageBlob,
      });
      ...
      if (!updateDataResponse.ok) {
        throw new Error("Failed to update data on the NestJS server");
      }
      ...
      toast({
        title: "Success",
        description:
          "Image processed and saved successfully! Check the result below.",
      });
